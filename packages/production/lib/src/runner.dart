...
class Runner {
  final String name;
  final PlatformConfigurer configureServer;
...
''';
  ...
  static void isolateMain(RunnerArgsWithId argsWithId) {
    ...
    zone.run(() async {
      ...
      var app = Application(reflector: args.reflector)
      ...
      if (args.options.ssl) {
        http = PlatformHttp.custom(app, startSharedSecure(securityContext),
            useZone: args.options.useZone);
      } else {
        http = PlatformHttp.custom(app, startShared,
            useZone: args.options.useZone);
      }
      ...
      if (args.options.http2) {
        ...
        var http2 = PlatformHttp2.custom(app, securityContext, startSharedHttp2,
            useZone: args.options.useZone);
