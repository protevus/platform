...
class Container {
    ...
    try {
      ...
      if (reflectedType is ReflectedClass) {
        ...
        try {
          ...
          var constructorNamed = <String, dynamic>{};
          for (var param in constructor.parameters.where((p) => p.isNamed)) {
            // First check parameter stack
            var override = getParameterOverride(param.name);
            if (override != null) {
              constructorNamed[param.name] = override;
              continue;
            }
            ...
            try {
              var value = make(param.type.reflectedType);
              constructorNamed[param.name] = value;
            } catch (e) {
              // Skip optional parameters that can't be resolved
              if (param.isRequired) {
                rethrow;
              }
            }
  ...
  /// Register a request class in the container
  /// ```dart
  /// container.registerRequest('BlogRequest', () => BlogRequest());
  /// ```
  void registerRequest(String name, Function callback) {
    if (name != 'dynamic') {
      // Convert the callback to a Container-aware factory function
      dynamic Function(Container) factory = (Container c) => callback();
      // Store in named singletons for string-based lookup
      _namedSingletons[name] = callback();
    }
  }

  /// Get a dependency by name
  /// ```dart
  /// var foo = container.getByName('Foo');
  /// ```
  dynamic getByName(String name) {
    ...
    // Fire before resolving callbacks with dynamic type
    _fireBeforeResolvingCallbacks(dynamic, [name]);

    // Search through container hierarchy
    while (search != null) {
      // Check singletons first
      if (search._namedSingletons.containsKey(name)) {
        instance = search._namedSingletons[name];
        instanceType = instance?.runtimeType;
        break;
      }
      // Then check factories
      if (search._factories.containsKey(name)) {
        instance = search._factories[name]!(this);
        instanceType = instance?.runtimeType;
        break;
      }
      search = search._parent;
    ...
    // Apply extenders and callbacks using actual type if available
    if (instanceType != null) {
      instance = _applyExtenders(instanceType, instance);
      _fireResolvingCallbacks(instance);
      _fireAfterResolvingCallbacks(instance);
    }

    // Handle named parameters from parameter stack
    if (_parameterStack.isNotEmpty) {
      var params = _parameterStack.last;
      var reflected = reflector.reflectInstance(instance);
      if (reflected != null) {
        for (var key in params.keys) {
          try {
            reflected.setField(key, params[key]);
          } catch (_) {
