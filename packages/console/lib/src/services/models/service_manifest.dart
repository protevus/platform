...
class HealthCheck {
  ...
  factory HealthCheck.fromYaml(Map<String, dynamic> yaml) {
    return HealthCheck(
      command: yaml['command']?.toString() ?? 'exit 0',
      interval: yaml['interval']?.toString() ?? '30s',
      timeout: yaml['timeout']?.toString() ?? '5s',
      retries: int.tryParse(yaml['retries']?.toString() ?? '') ?? 3,
    );
  }
...
class ConfigSchema {
  ...
  factory ConfigSchema.fromYaml(Map<String, dynamic> yaml) {
    return ConfigSchema(
      ...
      required:
          (yaml['required']?.toString() ?? 'false').toLowerCase() == 'true',
...
class ServiceManifest {
  ...
  factory ServiceManifest.fromYaml(Map<String, dynamic> yaml) {
    ...
    return ServiceManifest(
      ...
      supportedVersions: (yaml['versions'] as List<dynamic>?)
              ?.map((v) => v.toString())
              .toList() ??
          ['latest'],
      defaultVersion: yaml['default_version']?.toString() ?? 'latest',
      configSchema: configSchema,
      volumes: (yaml['volumes'] as List<dynamic>?)
              ?.map((v) => v.toString())
              .toList() ??
          [],
      environment: (yaml['environment'] as List<dynamic>?)
              ?.map((v) => v.toString())
              .toList() ??
          [],
